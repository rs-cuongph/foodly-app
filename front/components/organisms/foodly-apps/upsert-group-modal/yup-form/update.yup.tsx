...
export type UpdateGroupSchemaType = {
  name: string;
  date_range: {
    start: string | null;
    end: string | null;
  ...
  type: string;
  price: number | string | null;
  menu_items_deleted: string[];
};

export const useUpdateGroupSchema = () => {
  ...
  const schema = yupInstance.object().shape({
    ...
    menu_items: yupInstance.array().of(
      yupInstance.object().shape({
        id: yupInstance.string().label('id').optional().nullable(),
        name: yupInstance
          .string()
          .label('menu_name')
          .required()
    ...
    menu_items_deleted: yupInstance.array().optional(),
  ...
  return schema as yup.ObjectSchema<UpdateGroupSchemaType>;
};
...
const useUpdateGroupForm = () => {
  ...
  const form = useForm<UpdateGroupSchemaType>({
    mode: 'onChange',
    resolver: yupResolver(useUpdateGroupSchema()),
    defaultValues: {
      name: '',
      date_range: {
        start: startDate,
